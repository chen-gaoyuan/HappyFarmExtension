...
import { collectionFromPath, isEmptyPath } from '../nodes/Collection';
import { DOC, isCollection, isNode, isScalar, Node, NodeType, NODE_TYPE, ParsedNode, Range } from '../nodes/Node';
import { Pair } from '../nodes/Pair';
import type { Scalar } from '../nodes/Scalar';
import { toJS, ToJSContext } from '../nodes/toJS';
...
import { stringifyDocument } from '../stringify/stringifyDocument';
import { anchorNames, createNodeAnchors, findNewAnchor } from './anchors';
import { applyReviver } from './applyReviver';
import { createNode, CreateNodeContext } from './createNode';
...
export class Document<T extends Node = Node> {
    ...
    constructor(
        value?: unknown,
        replacer?: Replacer | (DocumentOptions & SchemaOptions & ParseOptions & CreateNodeOptions) | null,
        options?: DocumentOptions & SchemaOptions & ParseOptions & CreateNodeOptions,
    ) {
    ...
    clone(): Document<T> {
        ...
        copy.contents = isNode(this.contents) ? (this.contents.clone(copy.schema) as unknown as T) : this.contents;
    ...
    createNode(value: unknown, replacer?: Replacer | CreateNodeOptions | null, options?: CreateNodeOptions): Node {
        ...
        if (typeof replacer === 'function') {
            ...
            const keyToStr = (v: unknown) => typeof v === 'number' || v instanceof String || v instanceof Number;
    ...
    createPair<K extends Node = Node, V extends Node = Node>(
        ...
        const k = this.createNode(key, null, options) as unknown as K;
        const v = this.createNode(value, null, options) as unknown as V;
        return new Pair(k, v);
    }
    ...
    set(key: any, value: unknown): void {
        if (this.contents == null) {
            this.contents = collectionFromPath(this.schema, [key], value) as unknown as T;
    ...
    setIn(path: Iterable<unknown> | null, value: unknown): void {
        if (isEmptyPath(path)) this.contents = value as T;
            ...
            this.contents = collectionFromPath(this.schema, Array.from(path), value) as unknown as T;
        } else if (assertCollection(this.contents)) {
     ...
     */
    setSchema(version: '1.1' | '1.2' | 'next' | null, options: SchemaOptions = {}) {
