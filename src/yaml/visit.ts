...
import { isAlias, isCollection, isDocument, isMap, isNode, isPair, isScalar, isSeq, Node } from './nodes/Node';
...
export type asyncVisitorFn<T> = (
    ...
    node: T,
    path: readonly (Document | Node | Pair)[],
) => void | symbol | number | Node | Pair | Promise<void | symbol | number | Node | Pair>;
 ...
 * specific defined one will be used for each node.
 */
export async function visitAsync(node: Node | Document | null, visitor: asyncVisitor) {
    const visitor_ = initVisitor(visitor);
    if (isDocument(node)) {
