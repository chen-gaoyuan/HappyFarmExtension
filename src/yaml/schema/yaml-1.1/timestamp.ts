...
function parseSexagesimal<B extends boolean>(str: string, asBigInt?: B) {
    ...
    const num = (n: number | string) => (asBigInt ? (BigInt(n) as unknown as number) : Number(n));
    const res = parts
        .replace(/_/g, '')
        .split(':')
        .reduce((res, p) => res * num(60) + num(p), num(0));
    return (sign === '-' ? num(-1) * res : res) as B extends true ? number | bigint : number;
}
...
export const timestamp: ScalarTag & { test: RegExp } = {
    ...
    stringify: ({ value }) => (value as Date).toISOString().replace(/((T00:00)?:00)?\.000Z$/, ''),
};
