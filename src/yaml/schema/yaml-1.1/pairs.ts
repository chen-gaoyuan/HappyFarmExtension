...
import { isMap, isPair, isSeq, ParsedNode } from '../../nodes/Node';
    ...
    if (isSeq(seq)) {
        for (let i = 0; i < seq.items.length; ++i) {
            ...
            if (isPair(item)) continue;
                ...
                const pair = item.items[0] || new Pair(new Scalar(null) as Scalar.Parsed);
...
export function createPairs(schema: Schema, iterable: unknown, ctx: CreateNodeContext) {
    const { replacer } = ctx;
