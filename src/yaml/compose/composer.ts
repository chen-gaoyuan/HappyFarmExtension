...
import { ErrorCode, YAMLParseError, YAMLWarning } from '../errors';
import { isCollection, isPair, Range } from '../nodes/Node';
...
type ErrorSource = number | [number, number] | Range | { offset: number; source?: string };
...
export class Composer {
    ...
    *next(token: Token) {
        switch (token.type) {
            ...
            default:
                this.errors.push(
                    new YAMLParseError(getErrorPos(token), 'UNEXPECTED_TOKEN', `Unsupported token ${token.type}`),
                );
        }
    }
