...
type FlowScalarErrorHandler = (offset: number, code: ErrorCode, message: string) => void;
...
export function resolveFlowScalar(
    ...
    let value: string;
    const _onError: FlowScalarErrorHandler = (rel, code, msg) => onError(offset + rel, code, msg);
    switch (type) {
        case 'scalar':
            _type = Scalar.PLAIN;
...
function parseCharCode(source: string, offset: number, length: number, onError: FlowScalarErrorHandler) {
