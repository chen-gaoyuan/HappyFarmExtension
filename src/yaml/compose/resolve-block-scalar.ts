...
function parseBlockScalarHeader({ offset, props }: BlockScalar, strict: boolean, onError: ComposeErrorHandler) {
...
function splitLines(source: string) {
    ...
    const first = split[0];
    const m = first.match(/^( *)/);
    const line0: [string, string] = m?.[1] ? [m[1], first.slice(m[1].length)] : ['', first];
    const lines = [line0];
