...
import { isAlias, isCollection, isNode, isPair, isScalar, Node } from '../nodes/Node';
...
import { stringifyComment } from './stringifyComment';
...
export function createStringifyContext(doc: Document, options: ToStringOptions): StringifyContext {
...
function getTagObject(tags: Array<ScalarTag | CollectionTag>, item: Node) {
    if (item.tag) {
        ...
        if (match.length > 0) return match.find((t) => t.format === (item as Scalar).format) ?? match[0];
    }
...
function stringifyProps(node: Node, tagObj: ScalarTag | CollectionTag, { anchors, doc }: StringifyContext) {
