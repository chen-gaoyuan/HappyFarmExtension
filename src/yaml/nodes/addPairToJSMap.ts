...
import { createStringifyContext } from '../stringify/stringify';
import { isAlias, isMap, isNode, isScalar, isSeq } from './Node';
import type { Pair } from './Pair';
import { Scalar } from './Scalar';
import { toJS, ToJSContext } from './toJS';

const MERGE_KEY = '<<';

export function addPairToJSMap(
    ctx: ToJSContext | undefined,
    map: Map<unknown, unknown> | Set<unknown> | Record<string | number | symbol, unknown>,
    { key, value }: Pair,
) {
...
function stringifyKey(key: unknown, jsKey: unknown, ctx: ToJSContext | undefined) {
    if (jsKey === null) return '';
    if (typeof jsKey !== 'object') return String(jsKey);
    if (isNode(key) && ctx && ctx.doc) {
        const strCtx = createStringifyContext(ctx.doc, {});
        strCtx.anchors = new Set();
        for (const node of ctx.anchors.keys()) strCtx.anchors.add(node.anchor as 'string');
        strCtx.inFlow = true;
