...
import { ALIAS, isAlias, isCollection, isPair, Node, NodeBase, Range } from './Node';
...
export class Alias extends NodeBase {
    ...
    resolve(doc: Document): Scalar | YAMLMap | YAMLSeq | undefined {
        ...
        visit(doc, {
            Node: (_key: unknown, node: Node) => {
                ...
                if (node.anchor === this.source) found = node as any;
            },
        });
        return found;
    }
    ...
    toString(ctx?: StringifyContext, _onComment?: () => void, _onChompKeep?: () => void) {
...
function getAliasCount(doc: Document, node: unknown, anchors: ToJSContext['anchors']): number {
    if (isAlias(node)) {
