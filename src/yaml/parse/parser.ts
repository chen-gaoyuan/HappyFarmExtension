...
function isFlowToken(token: Token | null | undefined): token is FlowScalar | FlowCollection {
    switch (token?.type) {
        case 'alias':
        case 'scalar':
...
export class Parser {
    ...
    *end() {
    ...
    private flowScalar(type: 'alias' | 'scalar' | 'single-quoted-scalar' | 'double-quoted-scalar') {
        if (this.onNewLine) {
            let nl = this.source.indexOf('\n') + 1;
            while (nl !== 0) {
                this.onNewLine(this.offset + nl);
