import type { ComposeErrorHandler } from '../compose/composer';
import { resolveBlockScalar } from '../compose/resolve-block-scalar';
import { resolveFlowScalar } from '../compose/resolve-flow-scalar';
import { ErrorCode, YAMLParseError } from '../errors';
...
export function createScalarToken(
    ...
    switch (source[0]) {
        ...
        case '>': {
            ...
            const props: Token[] = [{ type: 'block-scalar-header', offset, indent, source: head }];
...
function setBlockScalarValue(token: Token, source: string) {
    ...
    if (token.type === 'block-scalar') {
        ...
        for (const key of Object.keys(token)) if (key !== 'type' && key !== 'offset') delete (token as any)[key];
